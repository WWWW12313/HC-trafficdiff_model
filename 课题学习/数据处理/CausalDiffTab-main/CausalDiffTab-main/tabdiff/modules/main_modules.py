...
class MLPDiffusion(nn.Module):
    def __init__(self, d_in, dim_t = 512, use_mlp=True, dropout=0.0):
        super().__init__()
        self.dim_t = dim_t
        ...
        if use_mlp:
            ...
            if dropout > 0:
                layers.append(nn.Dropout(p=dropout))
            layers.append(nn.Linear(dim_t, d_in))
            self.mlp = nn.Sequential(*layers)
        else:
            self.mlp = nn.Linear(dim_t, d_in)
...
class UniModMLP(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = MLPDiffusion(d_in, dim_t=dim_t, use_mlp=use_mlp, dropout=dropout)
