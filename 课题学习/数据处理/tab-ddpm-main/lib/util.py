...
def get_catboost_config(real_data_path, is_cv=False):
    ...
    cfg_path = Path(f'tuned_models/catboost/{ds_name}_cv.json')
    if cfg_path.exists():
        return load_json(cfg_path)

    default_path = Path('tuned_models/catboost/default_cv.json')
    print(
        f"[WARN] CatBoost tuned config not found for '{ds_name}', "
        f"fallback to '{default_path.as_posix()}'"
    )
