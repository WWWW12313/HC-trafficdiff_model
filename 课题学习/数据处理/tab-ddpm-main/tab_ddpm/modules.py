...
# ==========================================
# [新增] 因果掩码线性层 (Causal Masked Linear)
# ==========================================
class MaskedLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True):
        super().__init__(in_features, out_features, bias)
        # 这里做了一个自适应的下三角掩码
        # 确保对于因果排序后的特征，后面的特征预测时无法看到前面的信息
...
USE_MASKED_LINEAR = os.getenv("USE_MASKED_LINEAR", "0").strip().lower() in {"1", "true", "yes", "on"}
    ...
    class Block(nn.Module):
        ...
        def __init__(
            ...
            *,
            ...
            super().__init__()
            self.linear = MaskedLinear(d_in, d_out, bias) if USE_MASKED_LINEAR else nn.Linear(d_in, d_out, bias)
...
class MLPDiffusion(nn.Module):
    def __init__(self, d_in, num_classes, is_y_cond, rtdl_params, dim_t = 128):
        ...
        # ---------------------------------------------------------
        # 1. 恢复 Yandex 原版全连接 MLP（去噪网络需要看到所有特征）
        # ---------------------------------------------------------
