...
def _run_eval(tier: str, task_type: str | None = None, transfer_test: str | None = None) -> None:
    if transfer_test:
        cmd = [
            str(PYTHON),
            str(ROOT / "pipeline" / "run_transfer_eval.py"),
            "--real_test",
            transfer_test,
            "--output_tag",
            f"transfer_{tier}",
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="启用迁移评估并指定迁移测试集路径，例如 data/nyc_crash_2025_v2/test.csv",
    )
    parser.add_argument(
        "--skip_in_domain_eval",
        action="store_true",
        help="跳过域内评估，仅做迁移评估（需配合 --transfer_test）",
    )
    ...
    if not args.skip_eval and not args.skip_in_domain_eval:
        _run_eval(args.tier, args.task_type)

    if args.transfer_test:
        _run_eval(args.tier, args.task_type, transfer_test=args.transfer_test)
