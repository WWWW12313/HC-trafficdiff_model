...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        if len(num_classes) > 0:
            # ★ 使用 register_buffer 确保设备一致性
            num_classes_expanded = torch.from_numpy(
                np.concatenate([num_classes[i].repeat(num_classes[i]) for i in range(len(num_classes))])
            ).float()
            self.register_buffer('num_classes_expanded', num_classes_expanded.to(device))

            self.slices_for_classes = [np.arange(self.num_classes[0])]
            offsets = np.cumsum(self.num_classes)
            self.slices_for_classes.extend([
                np.arange(self.num_classes[i]) + offsets[i - 1] for i in range(1, len(self.num_classes))
            ])
            offset_boundaries = np.concatenate(([0], offsets)).astype(np.int64)
            self.register_buffer('offsets', torch.from_numpy(offset_boundaries).to(device))
        else:
            # 全数值模式：给这些变量赋空值，防止后续代码找不到变量
            self.register_buffer('num_classes_expanded', torch.tensor([], device=device))
            self.slices_for_classes = []
            self.register_buffer('offsets', torch.tensor([], dtype=torch.long, device=device))
        ...
        self.moment_matching_weight = float(moment_matching_weight)
        self.moment_matching_target_index = moment_matching_target_index
        ...
        self.register_buffer('posterior_variance', _posterior_variance.float().to(device))
        self.register_buffer('posterior_log_variance_clipped', torch.from_numpy(
            np.log(np.append(_posterior_variance[1], _posterior_variance[1:]))
        ).float().to(device))
    ...
    def gaussian_p_mean_variance(
        ...
        model_variance = torch.cat([self.posterior_variance[1].unsqueeze(0).to(x.device), self.posterior_variance[1:].to(x.device)], dim=0)
        # 使用后验方差（更稳定），原始代码用 β_t 会在高噪声时步放大误差
        ...
        # ★ 关键修复：裁剪 pred_xstart 防止高噪声时步的误差放大
        # QuantileTransformer 输出范围约 [-5.2, 5.2]，这里用稍宽的 [-6, 6]
        if clip_denoised:
    ...
    def predict_start(self, model_out, log_x_t, t, out_dict):
        # 新增：如果没有分类特征，直接返回与 model_out 形状一致的空 log_pred
        if self.num_classes.sum() == 0:
            return torch.empty_like(model_out)
    ...
    def mixed_loss(self, x, out_dict):
        ...
        # ★ 修复：必须传入加噪后的 x_in，而不是干净的 x
        # ★ 若启用了条件扩散，尝试从 out_dict 中获取 y
        y_cond = out_dict.get('y') if out_dict is not None else None
        model_out = self._denoise_fn(x_in, t, y=y_cond)

        model_out_num = model_out[:, :self.num_numerical_features]
        model_out_cat = model_out[:, self.num_numerical_features:]

        loss_multi = torch.zeros((1,), device=x.device).float()
        loss_gauss = torch.zeros((1,), device=x.device).float()
        loss_moment = torch.zeros((1,), device=x.device).float()
        if x_cat.shape[1] > 0:
        ...
        if x_num.shape[1] > 0:
            ...
            if self.moment_matching_weight > 0:
                if self.gaussian_parametrization == 'eps':
                    pred_xstart_num = self._predict_xstart_from_eps(x_num_t, t, model_out_num)
                else:
                    pred_xstart_num = model_out_num

                pred_xstart_num = torch.clamp(pred_xstart_num, -6.0, 6.0)

                target_idx = self.moment_matching_target_index
                if isinstance(target_idx, int) and 0 <= target_idx < x_num.shape[1]:
                    x_real_stats = x_num[:, target_idx:target_idx + 1]
                    x_pred_stats = pred_xstart_num[:, target_idx:target_idx + 1]
                else:
                    x_real_stats = x_num
                    x_pred_stats = pred_xstart_num

                mu_real = x_real_stats.mean(dim=0)
                mu_pred = x_pred_stats.mean(dim=0)
                std_real = x_real_stats.std(dim=0, unbiased=False)
                std_pred = x_pred_stats.std(dim=0, unbiased=False)

                mu_scale = std_real.detach() + 1e-3
                std_scale = std_real.detach() + 1e-3
                mu_diff = (mu_pred - mu_real) / mu_scale
                std_diff = (std_pred - std_real) / std_scale

                loss_moment = F.smooth_l1_loss(mu_diff, torch.zeros_like(mu_diff)) + F.smooth_l1_loss(std_diff, torch.zeros_like(std_diff))
                loss_gauss = loss_gauss + self.moment_matching_weight * loss_moment

                if isinstance(out_dict, dict):
                    out_dict['loss_moment'] = loss_moment.detach()
    ...
    def sample_ddim(self, num_samples, y_dist):
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            model_out_num = model_out[:, :self.num_numerical_features]
            model_out_cat = model_out[:, self.num_numerical_features:]
            z_norm = self.gaussian_ddim_step(model_out_num, z_norm, t, clip_denoised=True)
            if has_cat:
