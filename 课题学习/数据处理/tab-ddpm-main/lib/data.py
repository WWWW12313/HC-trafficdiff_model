...
def read_pure_data(path, split='train'):
    ...
    if os.path.exists(os.path.join(path, f'X_num_{split}.npy')):
        ...
        # Ensure numeric matrix is float-compatible for downstream np.isnan calls.
        if X_num.dtype.kind not in {'f', 'i', 'u', 'b'}:
            X_num = pd.DataFrame(X_num).apply(pd.to_numeric, errors='coerce').to_numpy(dtype=float)
        else:
            X_num = X_num.astype(float, copy=False)
