...
def main(args):
    ...
    # 获取密度绘图数据
    density_data = _generate_density_data(syn_data, real_data, info)

    # 保存数据为 JSON
    save_dir = f"eval/density_data/{dataname}"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    save_path = os.path.join(save_dir, sample_file_name.replace('.csv', '.json'))

    with open(save_path, 'w') as f:
        json.dump(density_data, f, indent=4)

    print(f"Density data saved to {save_path}")
...
def get_column_plot_data(real_data, synthetic_data, column_name, plot_type):
    real_vals = real_data[column_name].dropna()
    syn_vals = synthetic_data[column_name].dropna()
    ...
    if plot_type == 'distplot':
        ...
        real_kde = gaussian_kde(real_vals)(x_vals)
        syn_kde = gaussian_kde(syn_vals)(x_vals)

        return {
            "type": "continuous",
            "x": x_vals.tolist(),
            "real_y": real_kde.tolist(),
            "syn_y": syn_kde.tolist()
        }

    elif plot_type == 'bar':
        # 处理分类列：频数统计
        all_categories = sorted(set(real_vals.unique().tolist() + syn_vals.unique().tolist()))

        real_counter = dict(Counter(real_vals))
        syn_counter = dict(Counter(syn_vals))

        real_freq = [real_counter.get(cat, 0) for cat in all_categories]
        syn_freq = [syn_counter.get(cat, 0) for cat in all_categories]

        return {
            "type": "categorical",
            "categories": all_categories,
            "real_counts": real_freq,
            "syn_counts": syn_freq
        }
...
def _generate_density_data(self, syn_data_cp, real_data, info):
    """
    实际生成密度对比数据的逻辑
    """
    density_data = {}
    ...
    for i, col in enumerate(column_names):
        ...
        if plot_type == 'distplot' and (syn_data_cp[col] == syn_data_cp[col].iloc[0]).all():
            print(f"\n ALERT: the generated samples column_{i} with name '{col}' all has the same value of {syn_data_cp[col].iloc[0]} \n")
            syn_data_cp[col].iloc[0] += 1e-5  # 避免全一样值

        data = get_column_plot_data(real_data, syn_data_cp, col, plot_type)
        density_data[col] = data
