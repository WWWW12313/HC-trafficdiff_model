...
Example:
  python pipeline/run_external_baselines.py --tier balanced
"""
...
def _pick_smote_target(df: pd.DataFrame, info: dict) -> str:
    ...
    # Fallback: choose the highest-cardinality categorical-like feature.
    ...
    if not candidates:
        # Last fallback: any non-constant column.
        return max(
...
def _fit_sample_smote(
    ...
    # Decode categorical feature columns and y.
...
def _coerce_to_reference_schema(syn: pd.DataFrame, ref: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep only reference columns and add missing columns.
...
def main() -> None:
    ...
    # CTGAN 目标列分布修正：clip 到训练集 1%-99% 分位数，缓解极端值导致 R2 为负
